...
public class DepartmentPanel extends JPanel {
    ...
    private static final String[] COLUMNS = {
            "Department Name", "Head of Department", "No. of Lecturers", "No. of Students"
    };
    ...
    private void buildTable() {
        ...
        tableModel.addRow(new Object[]{"Software Engineering", "Dr. Amal Perera",  "2", "45"});
        tableModel.addRow(new Object[]{"Computer Science",      "Dr. Nimali Silva", "3", "60"});
    ...
    private JPanel buildTopBar() {
        ...
        // Left: heading
        JLabel heading = new JLabel("Departments");
    ...
    private void openAddDialog() {
        JDialog dlg = makeDialog("Add New Department", 420, 440);

        JTextField tfName      = inputField("");
        JTextField tfHead      = inputField("");
        JTextField tfLecturers = inputField("");
        JTextField tfStudents  = inputField("");

        dlg.add(buildForm(
                new String[]{"Department Name *", "Head of Department *", "No. of Lecturers", "No. of Students"},
                new JTextField[]{tfName, tfHead, tfLecturers, tfStudents}
        ), BorderLayout.CENTER);

        dlg.add(makeFooter(dlg, "Save", () -> {
            if (tfName.getText().trim().isEmpty() || tfHead.getText().trim().isEmpty()) {
                JOptionPane.showMessageDialog(dlg,
                        "Department Name and Head of Department are required.", "Validation",
                        JOptionPane.WARNING_MESSAGE);
                return;
            }
            //  Add new row to the department table
            tableModel.addRow(new Object[]{
                    tfName.getText().trim(),
                    tfHead.getText().trim(),
                    tfLecturers.getText().trim(),
                    tfStudents.getText().trim()
            });
            refreshCount();
            dlg.dispose();
        }), BorderLayout.SOUTH);
    ...
    private void openEditDialog() {
        ...
        if (row == -1) {
            JOptionPane.showMessageDialog(this,
                    "Please click on a department row first, then click Edit.",
                    "No Row Selected", JOptionPane.WARNING_MESSAGE);
            return;
        }

        JDialog dlg = makeDialog("Edit Department", 420, 440);

        // Pre-fill with selected row values
        JTextField tfName      = inputField((String) tableModel.getValueAt(row, 0));
        JTextField tfHead      = inputField((String) tableModel.getValueAt(row, 1));
        JTextField tfLecturers = inputField((String) tableModel.getValueAt(row, 2));
        JTextField tfStudents  = inputField((String) tableModel.getValueAt(row, 3));
        ...
        dlg.add(makeFooter(dlg, "Update", () -> {
            ...
            tableModel.setValueAt(tfHead.getText().trim(),      row, 1);
            tableModel.setValueAt(tfLecturers.getText().trim(), row, 2);
            tableModel.setValueAt(tfStudents.getText().trim(),  row, 3);
            dlg.dispose();
        }), BorderLayout.SOUTH);
    ...
    private void deleteSelected() {
        ...
        if (row == -1) {
            JOptionPane.showMessageDialog(this,
                    "Please click on a department row first, then click Delete.",
                    "No Row Selected", JOptionPane.WARNING_MESSAGE);
            return;
        }
    ...
    private void refreshCount() {
        if (countLabel != null)
            countLabel.setText("Showing " + tableModel.getRowCount() + " department(s)");
    }
