...
public class CourseManagement extends JFrame {
    ...
    /** Display label shown in the UI. Data is still fetched normally via StudentDAO. */
    static final String DISPLAY_STUDENT_LABEL = "CT2022014 - KUMARASINGHE K.M.B.S.S";
    ...
    public CourseManagement() {
        ...
        try {
            currentStudent = studentDAO.getStudent(CURRENT_STUDENT_ID);
        } catch (SQLException ex) {
            currentStudent = new StudentDAO.StudentInfo(CURRENT_STUDENT_ID, CURRENT_STUDENT_ID, "");
        }
    ...
    private JPanel buildSidebar() {
        ...
        sidebar.add(sidebarItem("\uD83C\uDFE0  Home", false));
        sidebar.add(Box.createVerticalStrut(4));
        sidebar.add(sidebarItem("\uD83D\uDCCA  Dashboard", false));
        sidebar.add(Box.createVerticalStrut(4));
        sidebar.add(sidebarItem("\uD83D\uDCDA  My courses", true));

        sidebar.add(Box.createVerticalGlue());

        JPanel logoutRow = sidebarItem("\u21AA  Logout", false);
    ...
    private JScrollPane buildMyCoursesView() {
        ...
        JLabel greeting = new JLabel("Hi, " + DISPLAY_STUDENT_LABEL + "! \uD83D\uDC4B");
    ...
    private void reloadCourses() {
        try {
            allCourses = courseDAO.getEnrolledCourses(CURRENT_STUDENT_ID);
        } catch (SQLException ex) {
    ...
    private void refreshSubmissionList(JPanel subListPanel, int courseId) {
        ...
        try {
            List<Submission> subs = submissionDAO.getSubmissions(courseId, CURRENT_STUDENT_ID);
            if (subs.isEmpty()) {
    ...
    private void submitAssignment(Course course, JPanel subListPanel) {
        ...
        try {
            ...
            submissionDAO.addSubmission(course.id, CURRENT_STUDENT_ID, selected.getName(), dest.getPath());
